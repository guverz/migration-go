...
func TestDeletedIncludes(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name: "nothing to collect",
			setup: func() string {
				tmpDir, err := os.MkdirTemp("", "test_missed_includes_*")
				if err != nil {
					t.Fatalf("failed to create dir: %v", err)
				}
				t.Cleanup(func() {
				...
				baseProjectName := "test-project-0.1.0"
				baseModuleName := "test-module-0.1.0"
				baseIncludeName := "test-include"

				for i := 1; i < 5; i++ {
					for j := 1; j < 4; j++ {
						includeName := fmt.Sprintf("%s-%v.%v.sql", baseIncludeName, i, j)
						projectIncludePath := filepath.Join(projectIncludes, includeName)
						moduleIncludePath := filepath.Join(moduleIncludes, includeName)
						projectNameUp := fmt.Sprintf("%s-%v-%v.up.sql", baseProjectName, i, j)
						projectNameDown := fmt.Sprintf("%s-%v-%v.down.sql", baseProjectName, i, j)
						projectFileUp := filepath.Join(projectMigrationsDir, projectNameUp)
						projectFileDown := filepath.Join(projectMigrationsDir, projectNameDown)
						...
						if err := os.WriteFile(moduleFileUp, []byte(fmt.Sprintf("# %s\n@includes/%s", moduleNameUp, includeName)), 0644); err != nil {
							t.Fatalf("error creating file: %v", err)
						}
						if err := os.WriteFile(moduleFileDown, []byte(fmt.Sprintf("# %s", moduleNameDown)), 0644); err != nil {
							t.Fatalf("error creating file: %v", err)
						...
						if err := os.WriteFile(projectFileUp, []byte(fmt.Sprintf("# %s\n#migration: %s;%s\n@includes/%s", projectNameUp, relativeModuleFileUp, concatMD5, includeName)), 0644); err != nil {
							t.Fatalf("error creating file: %v", err)
						}
						if err := os.WriteFile(projectFileDown, []byte(fmt.Sprintf("# %s\n#migration: %s;%s", projectNameDown, relativeModuleFileDown, concatMD5)), 0644); err != nil {
							t.Fatalf("error creating file: %v", err)
		...
		{
			name: "1 missing include in module",
			setup: func() string {
				tmpDir, err := os.MkdirTemp("", "test_missed_includes_*")
				if err != nil {
					t.Fatalf("failed to create dir: %v", err)
				...
				for i := 1; i < 10; i++ {
					for j := 1; j < 10; j++ {
						...
						default:
							...
							if err := os.WriteFile(projectIncludePath, []byte(""), 0644); err != nil {
								t.Fatalf("error creating file: %v", err)
							}
							if err := os.WriteFile(moduleIncludePath, []byte(""), 0644); err != nil {
								t.Fatalf("error creating file: %v", err)
							}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (len(rslts.DeletedIncludes) != 0) != tt.wantProjectErr {
				t.Errorf("deletedIncludes() found = %v, want %v", (len(rslts.DeletedIncludes) != 0), tt.wantProjectErr)
			}
			if collected != tt.wantCollectedCnt {
				t.Errorf("deletedIncludes() collected = %v, want %v", collected, tt.wantCollectedCnt)
			}
		})
	}
}

func TestMissedIncludes(t *testing.T) {
	tests := []struct {
		name             string
		setup            func() string
		wantCollectedCnt int
		...
		{
			name: "1 changed 1 missing include in project",
			setup: func() string {
				tmpDir, err := os.MkdirTemp("", "test_missed_includes_*")
				if err != nil {
					t.Fatalf("failed to create dir: %v", err)
				...
				for i := 1; i < 10; i++ {
					for j := 1; j < 10; j++ {
						...
						case i == 2 && j == 4:
							...
							if err := os.WriteFile(projectIncludePath, []byte("t"), 0644); err != nil {
								t.Fatalf("error creating file: %v", err)
							}
							if err := os.WriteFile(moduleIncludePath, []byte(""), 0644); err != nil {
								t.Fatalf("error creating file: %v", err)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("missedIncludes() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if (len(rslts.MissedIncludes) != 0) != tt.wantProjectErr {
				t.Errorf("missedIncludes() found = %v, want %v", (len(rslts.MissedIncludes) != 0), tt.wantProjectErr)
			}
			if collected != tt.wantCollectedCnt {
				t.Log(rslts.MissedIncludes)
				t.Errorf("missedIncludes() collected = %v, want %v", collected, tt.wantCollectedCnt)
			}
		})
	}
}
